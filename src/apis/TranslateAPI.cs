...
namespace LiveCaptionsTranslator.apis
{
    public static class TranslateAPI
    {
        ...
        /// <summary>
        /// 通义千问（阿里云 DashScope），使用 OpenAI 兼容接口。
        /// </summary>
        public static async Task<string> Qwen(string text, CancellationToken token = default)
        {
            var config = Translator.Setting["Qwen"] as QwenConfig;
            string language = QwenConfig.SupportedLanguages.TryGetValue(
                Translator.Setting.TargetLanguage, out var langValue) ? langValue : Translator.Setting.TargetLanguage;
            ...
            // 自动补全 DashScope API 路径：如果 URL 以 /v1 结尾但没有 /chat/completions，则自动添加
            ...
            {
                string responseString = await response.Content.ReadAsStringAsync();
                var responseObj = JsonSerializer.Deserialize<QwenConfig.Response>(responseString);
                var output = responseObj?.choices?[0].message?.content ?? "";
                return RegexPatterns.ModelThinking().Replace(output, "");
            }
            else
